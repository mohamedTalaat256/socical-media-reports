...
class RequestController extends Controller
{
    ...
    {
        $posts = Post::where('user',Auth::user()->id )
        ->join('sources', 'sources.id', '=', 'posts.source')
            ...
            ->orderBy('id', 'DESC')
            ->paginate(30);
        return view('user.requests.index', compact('posts'));
    }
    ...
    {
        event(new Message( 'request: 5454554'));
    ...
    public function viewPost($id)
    {
        ...
        return view('view_post', compact('post', 'sources'));
    }
    ...
    public function editPost($id)
    {
       ...
       // return $post;
        return view('edit_post', compact('post', 'sources'));
    }
    ...
    public function Update(Request $request)
    {
        ...
        if ($creator_image_file != '') {
            ...
            $creator_image_file->move('assets/images/creators', Carbon::now()->timestamp.$name);
            $image = $name;
            $data['creator_image'] = Carbon::now()->timestamp.$image;
        }
        ...
        return redirect()->route('posts_user')->with('success', 'post added successfully');
    }
    ...
    public function getPostsByKeyword($keyword)
    {
        $posts = Post::where('keyword', 'LIKE', '%' . $keyword . "%")
            ->join('sources', 'sources.id', '=', 'posts.source')
            ->join('users', 'users.id', '=', 'posts.user')
            ->select('posts.*', 'sources.name as s_name', 'sources.image as s_image', 'users.name as username')->paginate(50);

        $sources = Source::latest()->take(7)->get();
        return view('admin.posts.all_posts', compact('posts', 'sources'));
    }

    public function getPostsByRelatedTo($related_to)
    {

        $posts = Post::where('related_to', 'like', '%'. $related_to.'%')
            ->join('sources', 'sources.id', '=', 'posts.source')
            ->join('users', 'users.id', '=', 'posts.user')
            ->select('posts.*', 'sources.name as s_name', 'sources.image as s_image', 'users.name as username')->paginate(50);
    ...
    public function getPostsByStatusAndRelatedTo($status, $related_to)
    {
        if($status == 'NA'){
            $status='N/A';
        }
        ...
        $posts = Post::where('status', $status)
            ->where('related_to', 'like', '%'. $related_to.'%')
            ->join('sources', 'sources.id', '=', 'posts.source')
            ->join('users', 'users.id', '=', 'posts.user')
            ->select('posts.*', 'sources.name as s_name', 'sources.image as s_image', 'users.name as username')->paginate(50);
    ...
    public function filtterPostsBySourceDasboard($source)
    {
    ...
    public function filtterPostsBySource(Request $request)
    {
        if ($request->ajax()) {

            $posts = [];

            $posts = Post::where('source', $request->search_value)
                ->join('sources', 'sources.id', '=', 'posts.source')
                ->join('users', 'users.id', '=', 'posts.user')
                ->select('posts.*', 'sources.name as s_name', 'sources.image as s_image', 'users.name as username')->get();
            if ($posts) {
    ...
    public function posts_user()
    {
        ...
        if(Auth::user()->read_permission !=1 ){
            return redirect()->route('home')->with('danger', 'nothing to show to you');
        }
        $posts = Post::join('sources', 'sources.id', '=', 'posts.source')
            ->join('users', 'users.id', '=', 'posts.user')
            ->select('posts.*', 'sources.name as s_name', 'sources.image as s_image', 'users.name as username')
        ...
        return view('all_posts', compact('posts', 'sources'));
    }

    public function all_posts_of_user()
    {
        $posts = Post::where('user', Auth::user()->id)
            ->join('sources', 'sources.id', '=', 'posts.source')
            ->join('users', 'users.id', '=', 'posts.user')
    ...
    public function filtterPostsDateUser(Request $request)
    {
        $value = $request->value;
        $sources = Source::latest()->take(7)->get();

        if(Auth::user()->read_permission != 1){
            return redirect()->route('all_posts_of_user')->with('danger', 'you are not allowed to search posts');
        }
    ...
    public function searchPostsDateRangeUser(Request $request)
    {
        if(Auth::user()->read_permission != 1){
            return redirect()->route('all_posts_of_user')->with('danger', 'you are not allowed to search posts');
        }
