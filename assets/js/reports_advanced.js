...
function load(){
    var chart_url = window.location.origin+"/captinahmed/admin/reports_charts_data";
...
$('#search_posts').on('click', function(){
    var chart_url = base_url+"admin/advanced_reports_search_sources";
    var series =[];
    var labels =[];
    var colors =[];
    $.ajax({
        ...
        data: {
            ...
            'word': $('#related_to_posts').val(),
            '_token': '{{ csrf_token() }}'
        },
        success: function(data) {

            fillStatus_table(data, $('#status_select option:selected').text() +' posts from '+$('#posts_start_date').val()+' to '+ $('#posts_end_date').val() + 'related to '+ $('#related_to_posts').val())
