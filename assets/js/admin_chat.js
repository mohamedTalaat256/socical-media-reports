...
var KTLayoutChat = function () {
	...
	var _init = function (element) {
		...
		KTUtil.on(element, '#btn_chat_send', 'click', function(e) {
			_sendMessageFromAdmin(element);
           // getChats();
		});
	}


    var _sendMessageFromAdmin = function(element){
        ...
        $.ajax({
            type: 'post',
            url: base_url+'admin/sent_message_from_admin',
            data: {
...
function getComplainChat(complain_id){
    $.ajax({
        type: 'get',
        url: base_url+'admin/complain_messages',
        data: {
...
function fillDivByMessages(data){
    ...
    for(var row in messages){
        if( messages[row].sender_is_admin == 1){
            html += '<div class="d-flex flex-column mb-5 align-items-end">';
            html += '<div class="d-flex align-items-center">';
            html += '	<div>';
...
function getIncomingMessagesToAdmin(){
    $.ajax({
        type: 'get',
        url: base_url+'admin/get_incoming_messages_to_admin',
        data: {
            '_token': $('meta[name="csrf-token"]').attr('content')
        },
        success: function(data) {
...
function updateToSeen(id){
    $.ajax({
        type: 'post',
        url: base_url+'admin/update_message_status_to_seen',
        data: {
            'id': id,
            '_token': $('meta[name="csrf-token"]').attr('content')
        },
...
function getUpdatedSeenMessages(){


    $.ajax({
        type: 'get',
        url: base_url+'admin/get_updated_seen_messages_to_admin',
        data: {
            '_token': $('meta[name="csrf-token"]').attr('content')
        },
        success: function(data) {
...
function updateToSeenOnSender(id){
    $.ajax({
        type: 'post',
        url: base_url+'admin/update_to_seen_on_sender',
        data: {
            'id': id,
            '_token': $('meta[name="csrf-token"]').attr('content')
        },
...
setInterval(function() {
    getIncomingMessagesToAdmin();
    getUpdatedSeenMessages();
}, 1000);
